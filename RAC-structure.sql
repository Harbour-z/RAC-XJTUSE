CREATE SCHEMA IF NOT EXISTS `XJTUSE-RAC` DEFAULT CHARACTER SET utf8 ;
USE `XJTUSE-RAC` ;
...
CREATE TABLE user_info(
    ...
    `user_gender` tinyint unsigned NOT NULL  COMMENT '用户性别;1为男 2为女' ,
    `user_avatar` VARCHAR(255)   COMMENT '用户头像;头像资源地址' ,
    `is_deleted` tinyint unsigned NOT NULL DEFAULT 1 COMMENT '用户状态;1-正常 0-删除' ,
    `create_time` DATETIME NOT NULL  COMMENT '创建时间' ,
    `update_time` VARCHAR(255) NOT NULL  COMMENT '更新时间' ,
    `birthday` DATETIME   COMMENT '用户生日' ,
    `signature` VARCHAR(255)   COMMENT '个性签名' ,
    `email` tinyint unsigned   COMMENT '用户邮箱' ,
...
CREATE TABLE user_favorite(
    ...
    `create_time` DATETIME NOT NULL  COMMENT '' ,
    `update_time` DATETIME NOT NULL  COMMENT '' ,
    `merchant_tag` tinyint unsigned NOT NULL  COMMENT '对被收藏商户的tag(0-待探索 1-常去 2-备选 3-不感兴趣)' ,
    PRIMARY KEY (id)
)  COMMENT = '用户收藏表';
...
CREATE TABLE merchant_info(
    ...
    `status` tinyint unsigned NOT NULL  COMMENT '状态(0-禁用,1-启用,2-已冻结)' ,
    `create_time` DATETIME NOT NULL  COMMENT '' ,
    `update_time` DATETIME NOT NULL  COMMENT '' ,
...
CREATE TABLE merchant_qulification(
    ...
    `license_number` INT   COMMENT '营业许可证编号' ,
    ...
    `status` tinyint unsigned NOT NULL  COMMENT '审核进度((0-待审核,1-审核中,2-审核通过,3-审核不通过,4-已冻结))' ,
...
CREATE TABLE comment(
    ...
    `status` tinyint unsigned NOT NULL  COMMENT '状态(0-待审核,1-审核通过,2-审核不通过)' ,
...
CREATE TABLE merchant_content(
    ...
    `content_type` tinyint unsigned NOT NULL  COMMENT '类型(1-新品预告,2-优惠活动,3-店铺故事,4-厨师风采)' ,
...
CREATE TABLE admin_audit_log(
    `id` INT AUTO_INCREMENT AUTO_INCREMENT COMMENT '' ,
    `admin_id` bigint unsigned NOT NULL  COMMENT '' ,
    `audit_type` tinyint unsigned NOT NULL  COMMENT '审核类型(1-商家注册,2-商家信息修改,3-评论内容,4-商家动态)' ,
    `target_id` tinyint unsigned NOT NULL  COMMENT '审核对象ID(商家ID/评论ID/动态ID)' ,
    `old_status` tinyint unsigned NOT NULL  COMMENT '审核前状态' ,
